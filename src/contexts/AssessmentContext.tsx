...
export function AssessmentProvider({ children }: { children: ReactNode }) {
  ...
  const setToken = useCallback((token: string) => {
    setState((prev) => ({ ...prev, token, assessmentId: `ASM-${Date.now()}-${Math.random().toString(36).substr(2, 9)}` }));
  }, []);
  ...
  const answerQuestion = useCallback((questionId: string, value: AnswerValue, timeSpent: number) => {
    setState((prev) => {
      ...
      if (existingIndex >= 0) { const newAnswers = [...prev.answers]; newAnswers[existingIndex] = newAnswer; return { ...prev, answers: newAnswers }; }
  ...
  const nextQuestion = useCallback(() => {
    setState((prev) => ({ ...prev, currentQuestionIndex: Math.min(prev.currentQuestionIndex + 1, questions.length - 1) }));
  }, [questions.length]);
  ...
  const getProgress = useCallback(() => {
    ...
    return { current, total, stageProgress, stageTotal: 15 };
